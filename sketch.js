...
class Player {
  ...
  update() {
    ...
    if (this.state === 'RUN' || this.state === 'WALK') {
      let animSpeed = this.state === 'RUN' ? 0.6 : 0.4;
...
class Enemy {
  ...
  update() {
    ...
    if (this.state === 'DYING') {
      ...
      if (this.frameIndex >= this.totalDyingFrames) {
        this.dead = true;
      }
      return;
    }
  ...
  hit() {
    if (this.state === 'DYING' || player.state === 'HIDE') return;
    this.hp--;

    // Reaction Logic: Turn around and attack if shot from behind
    let playerRelativeDir = player.x > this.x ? 1 : -1;
    if (this.dir !== playerRelativeDir) {
      this.dir = playerRelativeDir;
      if (this.state !== 'SHOOT') {
        this.state = 'SHOOT';
        this.frameIndex = 0;
        this.firedThisCycle = false;
  ...
  draw() {
    ...
    if (this.state === 'DYING' && imgEnemyDying) {
      // User: "while looking to the left. Play this sprite sheet... If he's dying while looking to the right, flip the frames."
      // Assuming native is LEFT (as per user instruction "while looking to the left. Play this...").
      if (this.dir === 1) scale(-1, 1);

      let fIndex = floor(this.frameIndex);
      if (fIndex >= this.totalDyingFrames) fIndex = this.totalDyingFrames - 1;

      let fw = imgEnemyDying.width / this.dyingCols;
      let fh = imgEnemyDying.height / this.dyingRows;
      let col = fIndex % this.dyingCols;
      let row = floor(fIndex / this.dyingCols);

      image(imgEnemyDying, 0, 0, drawW, drawH, col * fw, row * fh, fw, fh);

    } else if (this.state === 'SHOOT' && imgEnemyShoot) {
